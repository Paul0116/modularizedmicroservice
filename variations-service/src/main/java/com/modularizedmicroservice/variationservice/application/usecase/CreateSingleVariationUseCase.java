...
public class CreateSingleVariationUseCase {
    ...
    public VariationResponse execute(CreateVariationRequest request) {

        if (validateIfExists(request).isPresent()) {
            throw new ResourceAlreadyExistsException("Variation with Barcode " + request.getBarcode() + " already exists.");
        }

        Variations variations = creatVariationBuilder(request);

        return variationResponseDtoConverter.convert(variationsRepository.save(variations));
    }

    public Optional<Variations> validateIfExists(CreateVariationRequest request) {
        return variationsRepository.findByCustomBarcode(request.getBarcode());
    }

    // Mapping method to convert CreateVariationRequest to Variations
    private Variations creatVariationBuilder(CreateVariationRequest request) {
